...
metric_props = {'mean': {'label': r'Mean $[cm^3/cm^3]$', 'min': 0.1, 'max': 0.5, 'cmap': 'rainbow'},
                'sd': {'label': r'Standard Deviation $[cm^3/cm^3]$', 'min': 0, 'max': 0.1, 'cmap': 'YlOrRd'}}
    ...
    for init in [0.125, 0.2, 0.35]:
        ...
        for metric in ['mean', 'sd']:
            ...
            if metric == 'mean':
                ...
                cm1 = plt.cm.get_cmap(metric_props[metric]['cmap'])

            gl = ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=True, linewidth=1, color='black', linestyle='--', alpha=0.3)
